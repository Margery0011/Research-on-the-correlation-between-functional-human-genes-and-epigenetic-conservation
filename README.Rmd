...
Use `entrez_link()` section to find transcript for a given gene. I will fetch cDNA sequences of those transcripts. First, get nucleotide IDs for refseq transcripts of two genes:

```{r,include=FALSE}
gene_ids <- gene_map$ENTREZID
linked_seq_ids <- entrez_link(dbfrom="gene", id=gene_ids, db="nucleotide")
linked_seq_ids$links
linked_transripts <- linked_seq_ids$links$gene_nuccore_refseqrna
head(linked_transripts)
```

Then get the sequences with `entrez_fetch`, acquire the fasta format by setting the `rettype``
